...
function makeAuditStub() {
  return {
    record: vi.fn().mockResolvedValue({ eventId: "evt-1", timestamp: new Date().toISOString() }),
...
describe("AuthService", () => {
  ...
  beforeEach(() => {
    ...
    service = new AuthService(repo, prisma, tenants, audit, bruteForce);
  });
  ...
  describe("login", () => {
    it("email + parola doğru → sessionToken + user + tenant", async () => {
      ...
      (repo.recordSuccessfulLogin as ReturnType<typeof vi.fn>).mockResolvedValue(undefined);
    ...
    it("bilinmeyen email → VET-AUTH-0002 (timing attack korumalı)", async () => {
      (repo.findUserByEmail as ReturnType<typeof vi.fn>).mockResolvedValue(null);
      const req: LoginRequest = { email: "ghost@nope.com", password: "whatever" };
      try {
    ...
    it("5 başarısız deneme sonrası hesap kilitlenir", async () => {
      ...
      (repo.findUserByEmail as ReturnType<typeof vi.fn>).mockResolvedValue(user);
      (repo.recordFailedLogin as ReturnType<typeof vi.fn>).mockResolvedValue(undefined);
  ...
  describe("logout", () => {
    it("session iptal edilir + audit yazılır", async () => {
      (repo.revokeSession as ReturnType<typeof vi.fn>).mockResolvedValue(undefined);
      const result = await service.logout("session-1", "user-1", makeCtx());
      expect(result.revokedAt).toBeTruthy();
      expect(repo.revokeSession).toHaveBeenCalledWith("session-1", "logout");
  ...
  describe("forgotPassword", () => {
    ...
    it("kullanıcı varsa token üretilir", async () => {
      const user = makeUser();
      (repo.findUserByEmail as ReturnType<typeof vi.fn>).mockResolvedValue(user);
      (repo.revokePasswordResets as ReturnType<typeof vi.fn>).mockResolvedValue(0);
  ...
  describe("resetPassword", () => {
    it("geçersiz token → VET-AUTH-0004", async () => {
      (repo.findPasswordResetByTokenHash as ReturnType<typeof vi.fn>).mockResolvedValue(null);
    ...
    it("süresi dolmuş token → VET-AUTH-0004", async () => {
      (repo.findPasswordResetByTokenHash as ReturnType<typeof vi.fn>).mockResolvedValue({
        id: "reset-1",
        userId: "user-1",
        tokenHash: "h",
    ...
    it("başarılı reset → parola güncellenir + tüm session'lar iptal", async () => {
      ...
      (repo.markPasswordResetUsed as ReturnType<typeof vi.fn>).mockResolvedValue(undefined);
      ...
      expect(result.sessionToken).toBeTruthy();
      expect(repo.revokeAllSessions).toHaveBeenCalledWith(user.id, "password_reset");
    });
  });
  ...
  describe("inviteUser", () => {
    it("bekleyen davet varsa VET-AUTH-0005", async () => {
      (prisma.userInvitation.findFirst as ReturnType<typeof vi.fn>).mockResolvedValue({
        id: "inv-pending",
        status: "pending",
      });
    ...
    it("yeni davet oluşturur + audit", async () => {
      (prisma.userInvitation.findFirst as ReturnType<typeof vi.fn>).mockResolvedValue(null);
      ...
      const req: InviteUserRequest = { email: "x@y.com", role: "STAFF" };
      const result = await service.inviteUser("tenant-1", req, "admin-1", makeCtx());
  ...
  describe("acceptInvitation", () => {
    it("geçersiz davet → VET-AUTH-0005", async () => {
      (repo.findInvitationByTokenHash as ReturnType<typeof vi.fn>).mockResolvedValue(null);
    ...
    it("süresi dolmuş davet → expired + VET-AUTH-0005", async () => {
      (repo.findInvitationByTokenHash as ReturnType<typeof vi.fn>).mockResolvedValue({
        id: "inv-1",
        tenantId: "tenant-1",
        email: "x@y.com",
        role: "STAFF",
      ...
      expect(repo.updateInvitation).toHaveBeenCalledWith("inv-1", {
        status: "expired",
      });
    });
    ...
    it("yeni kullanıcı oluşturur + membership atar + session açılır", async () => {
      ...
      (prisma.userTenantMembership.upsert as ReturnType<typeof vi.fn>).mockResolvedValue({});
      (repo.updateInvitation as ReturnType<typeof vi.fn>).mockResolvedValue({});
      (repo.createSession as ReturnType<typeof vi.fn>).mockResolvedValue({
        id: "session-1",
        userId: "user-new",
  ...
  describe("me", () => {
    it("kullanıcı + session + memberships döner", async () => {
      const user = makeUser();
      (repo.findUserById as ReturnType<typeof vi.fn>).mockResolvedValue(user);
      (prisma.userSession.findUnique as ReturnType<typeof vi.fn>).mockResolvedValue({
        id: "session-1",
        userId: user.id,
        tokenHash: "h",
        expiresAt: new Date(Date.now() + 60_000),
      ...
      (prisma.userTenantMembership.findMany as ReturnType<typeof vi.fn>).mockResolvedValue([
        {
          tenantId: "tenant-1",
          role: "OWNER",
          status: "active",
          tenant: { id: "tenant-1", slug: "pilot", name: "Pilot" },
        },
      ]);
  ...
  describe("switchTenant", () => {
    ...
    it("aktif üyelik varsa tenantId + role döner", async () => {
      ...
      const result = await service.switchTenant("user-1", { tenantSlug: "vet-2" });
