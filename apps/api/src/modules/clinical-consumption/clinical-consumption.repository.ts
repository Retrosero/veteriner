...
export class ClinicalConsumptionRepository {
  ...
  ): Promise<ClinicalConsumptionRecord> {
  ...
  public async persistedById(
    tenantId: string,
    id: string,
  ): Promise<ClinicalConsumptionRecord | null> {
    if (!this.prisma) return this.findById(tenantId, id);
    const row = await this.inTenant(tenantId, (tx) =>
      tx.clinicalConsumptionRecord.findFirst({ where: { tenantId, id } }),
    );
    return row ? this.map(row) : null;
  }
  public async persistedByContextRef(
    tenantId: string,
    contextRefId: string,
  ): Promise<ClinicalConsumptionRecord[]> {
    if (!this.prisma) return this.listByContextRef(tenantId, contextRefId);
    const rows = await this.inTenant(tenantId, (tx) =>
      tx.clinicalConsumptionRecord.findMany({
        where: { tenantId, contextRefId },
      }),
    );
  ...
  public async persistedSearch(
    tenantId: string,
    f: ClinicalConsumptionSearchFilters,
  ): Promise<{ items: ClinicalConsumptionRecord[]; total: number }> {
    if (!this.prisma) return this.search(tenantId, f);
    const where: Prisma.ClinicalConsumptionRecordWhereInput = {
      tenantId,
      ...(f.context ? { context: f.context } : {}),
      ...(f.contextRefId ? { contextRefId: f.contextRefId } : {}),
    ...
    const [rows, total] = await this.inTenant(tenantId, (tx) =>
      Promise.all([
        ...
        tx.clinicalConsumptionRecord.count({ where }),
      ]),
    );
    return { items: rows.map((r) => this.map(r)), total };
  }
  public async persistedCancel(
    tenantId: string,
    id: string,
    patch: Pick<
      ClinicalConsumptionRecord,
      "status" | "cancelledAt" | "cancelledBy" | "cancelReason"
    >,
  ): Promise<ClinicalConsumptionRecord | null> {
    if (!this.prisma) {
    ...
    const out = await this.inTenant(tenantId, (tx) =>
      tx.clinicalConsumptionRecord.updateMany({
        where: { tenantId, id },
        data,
      }),
    );
  ...
  public async cancelWithReversals(
    tenantId: string,
    id: string,
    cancelledBy: string,
    cancelReason: string,
    ...
    return this.inTenant(tenantId, async (tx) => {
      const rec = await tx.clinicalConsumptionRecord.findFirst({
        where: { tenantId, id },
      });
      if (!rec) return null;
      const movementIds = rec.stockMovementIds as unknown as string[];
      const originals = await tx.stockMovementRecord.findMany({
        where: { tenantId, id: { in: movementIds } },
      });
      if (originals.length !== movementIds.length)
        throw new Error("Klinik tüketim stok hareketi bulunamadı");
      const prior = await tx.stockMovementRecord.findMany({
        where: { tenantId, reversesMovementId: { in: movementIds } },
      ...
      for (const original of originals) {
        ...
        const quantity = original.quantity.startsWith("-")
          ? original.quantity.slice(1)
          : `-${original.quantity}`;
        await tx.stockMovementRecord.create({
          data: {
            id: `stmv-${tenantId.slice(0, 8)}-${randomUUID()}`,
            tenantId,
            type: "reversal",
            ...
            sourceType: "clinical_consumption_cancel",
            sourceId: id,
            reversesMovementId: original.id,
            reason: `clinical_consumption_cancel:${id}:${cancelReason}`,
            occurredAt: now,
            notes: null,
            createdAt: now,
      ...
      const update = await tx.clinicalConsumptionRecord.updateMany({
      ...
      const row = await tx.clinicalConsumptionRecord.findFirst({
        where: { tenantId, id },
      });
      return row ? this.map(row) : null;
    });
  ...
  private map(row: DbConsumption): ClinicalConsumptionRecord {
    return {
      id: row.id,
      tenantId: row.tenantId,
      context: row.context as ClinicalConsumptionRecord["context"],
      contextRefId: row.contextRefId,
      patientId: row.patientId,
      lines: row.lines as unknown as ClinicalConsumptionRecord["lines"],
      notes: row.notes,
      status: row.status as ClinicalConsumptionRecord["status"],
