...
export class PetshopSaleReturnsRepository {
  ...
  public async persistReturnWithLines(
    ret: PetshopSaleReturnRecord,
    lines: PetshopSaleReturnLineRecord[],
  ): Promise<void> {
    if (!this.prisma) {
      if (!this.byId.has(ret.id)) this.insert(ret);
      for (const line of lines) {
        if (!this.lineById.has(line.id)) this.insertLine(line);
      }
      return;
    }
    await this.inTenant(ret.tenantId, async (tx) => {
      await tx.petshopSaleReturnRecord.create({ data: this.returnData(ret) });
      if (lines.length)
        await tx.petshopSaleReturnLineRecord.createMany({
  ...
  public async persistedById(
    tenantId: string,
    id: string,
  ): Promise<PetshopSaleReturnRecord | null> {
    if (!this.prisma) return this.findById(tenantId, id);
    const row = await this.inTenant(tenantId, (tx) =>
      tx.petshopSaleReturnRecord.findFirst({ where: { tenantId, id } }),
    );
  ...
  public async persistedLines(
    tenantId: string,
    returnId: string,
  ): Promise<PetshopSaleReturnLineRecord[]> {
    if (!this.prisma) return this.listLinesByReturn(tenantId, returnId);
    const rows = await this.inTenant(tenantId, (tx) =>
      tx.petshopSaleReturnLineRecord.findMany({
        where: { tenantId, returnId },
        orderBy: { createdAt: "asc" },
      }),
    );
    return rows.map((row) => this.mapLine(row));
  }
  public async persistedByOriginalSale(
    tenantId: string,
    originalSaleId: string,
  ): Promise<PetshopSaleReturnRecord[]> {
    if (!this.prisma)
      return this.listReturnsByOriginalSale(tenantId, originalSaleId);
    const rows = await this.inTenant(tenantId, (tx) =>
      tx.petshopSaleReturnRecord.findMany({
        where: { tenantId, originalSaleId },
        orderBy: { createdAt: "asc" },
      }),
    );
    return rows.map((row) => this.mapReturn(row));
  }
  public async persistedSearch(
    tenantId: string,
    f: PetshopSaleReturnSearchFilters,
  ): Promise<{ items: PetshopSaleReturnRecord[]; total: number }> {
    if (!this.prisma) return this.search(tenantId, f);
    const where: Prisma.PetshopSaleReturnRecordWhereInput = {
      tenantId,
      ...(f.status ? { status: f.status } : {}),
      ...(f.originalSaleId ? { originalSaleId: f.originalSaleId } : {}),
      ...(f.customerOwnerId ? { customerOwnerId: f.customerOwnerId } : {}),
      ...(f.customerPatientId
        ? { customerPatientId: f.customerPatientId }
        : {}),
      ...(f.refundMethod ? { refundMethod: f.refundMethod } : {}),
      ...(f.search?.trim()
        ? {
            OR: [
              { id: { contains: f.search.trim() } },
              { originalSaleId: { contains: f.search.trim() } },
              { reason: { contains: f.search.trim(), mode: "insensitive" } },
            ],
          }
        : {}),
    };
    return this.inTenant(tenantId, async (tx) => {
      const [items, total] = await Promise.all([
        ...
        tx.petshopSaleReturnRecord.count({ where }),
      ]);
      return { items: items.map((r) => this.mapReturn(r)), total };
    });
  }
  public async persistedUpdate(
    tenantId: string,
    id: string,
    p: PetshopSaleReturnPatch,
  ): Promise<PetshopSaleReturnRecord | null> {
    if (!this.prisma) return this.update(tenantId, id, p);
    const data: Prisma.PetshopSaleReturnRecordUpdateManyMutationInput = {
      ...(p.status !== undefined ? { status: p.status } : {}),
      ...(p.refundMethod !== undefined ? { refundMethod: p.refundMethod } : {}),
      ...(p.totalAmount !== undefined ? { totalAmount: p.totalAmount } : {}),
      ...(p.globalDiscountPercent !== undefined
    ...
    const x = await this.inTenant(tenantId, (tx) =>
      tx.petshopSaleReturnRecord.updateMany({ where: { tenantId, id }, data }),
    );
  ...
  private returnData(
    r: PetshopSaleReturnRecord,
  ): Prisma.PetshopSaleReturnRecordUncheckedCreateInput {
    return {
      ...r,
  ...
  private lineData(
    r: PetshopSaleReturnLineRecord,
  ): Prisma.PetshopSaleReturnLineRecordUncheckedCreateInput {
    return {
      ...r,
      createdAt: new Date(r.createdAt),
      updatedAt: new Date(r.updatedAt),
  ...
  private mapReturn(r: DbReturn): PetshopSaleReturnRecord {
    return {
      ...r,
      status: r.status as PetshopSaleReturnStatus,
      refundMethod: r.refundMethod as PetshopPaymentMethod,
  ...
  private mapLine(r: DbLine): PetshopSaleReturnLineRecord {
    return {
      ...r,
      createdAt: r.createdAt.toISOString(),
      updatedAt: r.updatedAt.toISOString(),
