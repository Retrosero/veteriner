...
export class KasaRepository {
  ...
  public async persist(record:KasaEntryRecord):Promise<KasaEntryRecord>{if(!this.prisma)return this.insert(record);const row=await this.inTenant(record.tenantId,tx=>tx.kasaEntryRecord.create({data:{...record,amountSigned:record.amountSigned,occurredAt:new Date(record.occurredAt)}}));return this.map(row);}
  public async persistedBalance(tenantId:string,account:KasaEntryRecord["account"]):Promise<string>{if(!this.prisma)return this.getBalance(tenantId,account);const rows=await this.inTenant(tenantId,tx=>tx.kasaEntryRecord.findMany({where:{tenantId,account},select:{amountSigned:true}}));return scaledBigIntToReversalAmount(rows.reduce((sum,row)=>sum+(reversalAmountToScaled(row.amountSigned.toString())??BigInt(0)),BigInt(0)));}
  public async persistedListForReference(tenantId:string,referenceType:KasaEntryRecord["referenceType"],referenceId:string):Promise<KasaEntryRecord[]>{if(!this.prisma)return this.listForReference(tenantId,referenceType,referenceId);const rows=await this.inTenant(tenantId,tx=>tx.kasaEntryRecord.findMany({where:{tenantId,referenceType,referenceId},orderBy:{occurredAt:"asc"}}));return rows.map(row=>this.map(row));}
  public async persistedListForSessionRange(tenantId:string,_branchId:string,openedAt:string,closedAtOrNow:string):Promise<KasaEntryRecord[]>{if(!this.prisma)return this.listForSessionRange(tenantId,_branchId,openedAt,closedAtOrNow);const rows=await this.inTenant(tenantId,tx=>tx.kasaEntryRecord.findMany({where:{tenantId,occurredAt:{gte:new Date(openedAt),lte:new Date(closedAtOrNow)}},orderBy:{occurredAt:"asc"}}));return rows.map(row=>this.map(row));}
  ...
  private map(row:DbKasa):KasaEntryRecord{return{...row,account:row.account as KasaEntryRecord["account"],direction:row.direction as KasaEntryRecord["direction"],source:row.source as KasaEntryRecord["source"],referenceType:row.referenceType as KasaEntryRecord["referenceType"],method:row.method as KasaEntryRecord["method"],amountSigned:row.amountSigned.toString(),occurredAt:row.occurredAt.toISOString()};}
