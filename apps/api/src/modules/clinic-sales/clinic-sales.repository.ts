...
export class ClinicSalesRepository {
  ...
  public async persistSaleWithLines(
    sale: ClinicSaleRecord,
    lines: ClinicSaleLineRecord[],
  ): Promise<void> {
    if (!this.prisma) {
      if (!this.byId.has(sale.id)) this.insert(sale);
      for (const line of lines)
        if (!this.lineById.has(line.id)) this.insertLine(line);
      return;
    }
    await this.inTenant(sale.tenantId, async (tx) => {
      await tx.clinicSaleRecord.create({
        data: {
          ...sale,
          completedAt: sale.completedAt ? new Date(sale.completedAt) : null,
      ...
      if (lines.length)
        await tx.clinicSaleLineRecord.createMany({
          data: lines.map((line) => ({
            ...line,
  ...
  public async persistedLines(
    ...
    const rows = await this.inTenant(tenantId, (tx) =>
      tx.clinicSaleLineRecord.findMany({
  ...
  public async persistedUpdate(
    ...
    const data: Prisma.ClinicSaleRecordUpdateManyMutationInput = {
      ...
      ...(p.globalDiscountPercent !== undefined
        ...
        : {}),
      ...(p.netAmount !== undefined ? { netAmount: p.netAmount } : {}),
    ...
    const x = await this.inTenant(tenantId, (tx) =>
      tx.clinicSaleRecord.updateMany({ where: { tenantId, id }, data }),
    );
  ...
  public async persistedUpdateWithLines(
    ...
    if (!this.prisma) {
      const updated = this.update(tenantId, id, p);
      if (!updated) return null;
      for (const old of this.listLinesBySale(tenantId, id))
        this.lineById.delete(old.id);
      this.linesBySale.set(id, []);
    ...
    const changed = await this.inTenant(tenantId, async (tx) => {
      const result = await tx.clinicSaleRecord.updateMany({
        where: { tenantId, id, status: "draft" },
        data,
      });
      if (!result.count) return false;
      await tx.clinicSaleLineRecord.deleteMany({
    ...
    return changed ? this.persistedById(tenantId, id) : null;
  }
  public async persistedSearch(
    tenantId: string,
    f: ClinicSaleSearchFilters,
  ): Promise<{ items: ClinicSaleRecord[]; total: number }> {
    if (!this.prisma) return this.search(tenantId, f);
    const where: Prisma.ClinicSaleRecordWhereInput = {
      tenantId,
      ...
      ...(f.search?.trim()
        ? {
            OR: [
              ...
              { sourceId: { contains: f.search.trim() } },
              { notes: { contains: f.search.trim(), mode: "insensitive" } },
            ],
          }
        : {}),
    };
    return this.inTenant(tenantId, async (tx) => {
      const [items, total] = await Promise.all([
        tx.clinicSaleRecord.findMany({
          where,
          orderBy: { createdAt: f.sort ?? "desc" },
          skip: f.offset,
          take: f.limit,
        }),
        tx.clinicSaleRecord.count({ where }),
      ]);
  ...
  private mapSale(row: DbSale): ClinicSaleRecord {
    return {
      ...row,
      status: row.status as ClinicSaleStatus,
      sourceType: row.sourceType as ClinicSaleSourceType,
  ...
  private mapLine(row: DbLine): ClinicSaleLineRecord {
    return {
      ...row,
      createdAt: row.createdAt.toISOString(),
      updatedAt: row.updatedAt.toISOString(),
