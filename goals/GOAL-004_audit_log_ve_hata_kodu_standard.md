# GOAL-004 — Audit, log ve hata kodu standardı
...
Sistem genelinde kullanılacak audit, sistem logu, entegrasyon logu, background job logu ve güvenlik logu standartlarını tasarla.

Her log türü için zorunlu alanları, PII maskeleme kurallarını, severity seviyelerini, retention yaklaşımını ve error code formatını tanımla.

Örnek hata kodu biçimi:
`VET-[MODUL]-[NUMARA]`

Request/correlation ID standardını belirle. Hangi olayların audit oluşturacağını dokümante et.

Henüz Superadmin ekranı geliştirme; ortak log sözleşmesini ve yardımcı paket iskeletini oluştur.
