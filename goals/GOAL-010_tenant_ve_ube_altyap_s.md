...
Tenant ve şube modülünü geliştir.

Kurallar:

- Her tenant benzersiz slug ve durum bilgisine sahip olmalı.
- Pilot tenant tek şube ile başlayacak, ancak veri modeli çoklu şubeye uygun olmalı.
- Tenant'a ait tüm tablolarda `tenant_id` zorunlu olmalı.
- Şubeye bağlı tablolarda `branch_id` bulunmalı.
- Tenant oluşturma yalnızca SUPERADMIN tarafından yapılmalı.
- Tenant kapatma fiziksel veri silmemeli.

PostgreSQL RLS policy'lerini ekle ve cross-tenant negatif testlerini yaz.
