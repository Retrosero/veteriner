...
const tiers = ["pilot", "production", "critical"] as const satisfies readonly BackupTier[];
...
interface TierReport {
  ...
  rpoMinutes: number;
  rtoMinutes: number;
  retention: { daily: number; weekly: number; monthly: number; coldDays: number };
...
for (const tier of tiers) {
  ...
  if (cfg.rpoMinutes > cfg.rtoMinutes) {
    validationIssues.push(`${tier}: RPO (${cfg.rpoMinutes}) > RTO (${cfg.rtoMinutes})`);
  }
  if (cfg.retentionDaily < 7) validationIssues.push(`${tier}: daily retention < 7`);
  if (cfg.retentionWeekly < 4) validationIssues.push(`${tier}: weekly retention < 4`);
  if (cfg.retentionMonthly < 12) validationIssues.push(`${tier}: monthly retention < 12`);
}
...
const out = {
  ...
  validation: { issues: validationIssues, allOk: validationIssues.length === 0 },
  followups: [
...
const reportFile = join(reportDir, `dry-run-${now.toISOString().slice(0, 10)}.json`);
...
for (const t of reports) {
  ...
  lines.push(`  Upload: ${t.config.backend} (${t.config.encryption}, cold=${t.config.coldTier})`);
  lines.push(`  Output: ${t.outputFile}`);
  lines.push(`  Object key: ${t.objectKey ?? "(local only)"}`);
  lines.push(`  Retention: ${t.retention.daily} daily, ${t.retention.weekly} weekly, ${t.retention.monthly} monthly`);
  lines.push(`  Cold storage: ${t.config.coldTier} (${t.retention.coldDays} gunden sonra)`);
  lines.push("");
}
