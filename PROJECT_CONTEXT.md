# Proje Bağlamı
  ...
  - GOAL-086 ✅ Gözlem ve taburcu özeti (tamamlandı — 2026-07-30, core: e2f1c6, docs/i18n: bu commit). 8 endpoint (observations + discharge-summary CRUD + finalize/amend/portal-share); observation 4 kategori (vitals/intake_output/behavior/general); discharge summary state draft→finalized→amended (append-only); portal paylaşım sınırlı süreli token (default 30 gün, KVKK). Audit udit:observation.create + udit:discharge_summary.*. Yatış (GOAL-084) entegre. **FAZ-8 TAMAMLANDI (GOAL-080 → 086, 7/7 docs).**
  - GOAL-085 ✅ Yatış order ve uygulama kayıtları (tamamlandı — 2026-07-30, core: 72f2f8, docs/i18n: bu commit). 9 API doc (8 endpoint + 1 shared schedules list); 6 order type (medication/fluid_therapy/feeding/monitoring/procedure/other); order state active→completed|cancelled; schedule state pending→applied|skipped; apply skipReason 6 enum; cancel pending schedules skipped. Faz 8 reaktif hook medication apply → clinical_usage stok düşümü. Audit udit:hospitalization_order.*.
  - GOAL-084 ✅ Yatış ve kafes yönetimi (tamamlandı — 2026-07-30, core: 3ae31f, docs/i18n: bu commit). 13 endpoint (cages×4 + hospitalizations×9); 5 kafes türü (standard/isolation/intensive_care/recovery/quarantine); state planned→admitted→discharged|cancelled; CageAssignment (1 aktif); admit'te kafes atanır (doluysa 409); cancel dmitted için force=true. Audit udit:cage.* + udit:cage_assignment.* + udit:hospitalization.*. Taburcu özeti (GOAL-086) entegre.
  - GOAL-083 ✅ Operasyon notu (tamamlandı — 2026-07-30, core: d18d45a, docs/i18n: bu commit). 8 endpoint (POST/GET list/GET :id/PATCH/POST :id/team/materials/finalize/amend); state draft→finalized→amended; 2 alt kayıt (team, materials); plan in_progress zorunlu (VET-OPNOTE-0003); ikinci not reddi (VET-OPNOTE-0004); amend append-only (eski not korunur). Audit udit:operation_note.*. Faz 8 reaktif hook ile materials → clinical_usage stok düşümü.
  - GOAL-082 ✅ Anestezi takip (tamamlandı — 2026-07-30, core: 41af5dd, docs/i18n: bu commit). 8 endpoint (POST/GET list/GET :id/POST :id/medications/vitals/complications/staff/finalize); state draft→finalized; 4 alt kayıt tipi (medications/vitals/complications/staff); anesthesiaType (local/regional/general/sedation) + airwayType (none/face_mask/lma/endotracheal_tube); plan in_progress zorunlu (VET-ANESTHESIA-0003); ikinci kayıt reddi (VET-ANESTHESIA-0004). Audit udit:anesthesia.*. Ameliyat (GOAL-080) entegre.
